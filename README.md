...
The `cdd-go` compiler leverages a unified architecture to support various facets of API and code lifecycle management. For a deep dive into the compiler's design, see [ARCHITECTURE.md](ARCHITECTURE.md).
...
- **Model Context Protocol (MCP)**: Native integration for MCP stdio client and server capabilities. Expose Go functions as MCP tools or integrate OpenAPI/SDK workflows via the standard JSON-RPC protocol over standard input/output.

**Uncommon Features:**

`cdd-go` supports standard CDD features.

## CLI Options

```text
Usage: cdd-go [OPTIONS] <COMMAND>
```
