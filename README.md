...
- **Model Context Protocol (MCP)**: Native integration that exposes the compiler's functionality directly to AI tools via the `cdd-go mcp` standard stdio server, and scaffolded outputs (`--mcp`).
- **Integration Tests & Mocks**: Automatic test suite generation and fake mock endpoints using `--tests`.
- **Project Scaffolding**: Built-in functionality to scaffold standard Go module layouts, including GitHub Actions pipelines (unless disabled via `--no-github-actions` and `--no-installable-package`).
- **JSON-RPC Server**: Features a `serve_json_rpc` subcommand to support external editor plugins and IDE integrations natively.
