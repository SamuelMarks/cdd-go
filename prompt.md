...
The integration tests for this toolchain currently fail when running natively against the Stoplight Prism mock server (`./local-test.sh only-test` with `ONLY_TEST=cdd-go`).

## Instructions
1. Run the test locally for this specific language using: 
   `cd .. && ONLY_TEST=cdd-go ./local-test.sh only-test`
...
4. Verify your changes by ensuring `cd .. && ONLY_TEST=cdd-go ./local-test.sh only-test` passes successfully (including the coverage validation and chaos saboteur audits).
...
The Go SDK fails on several tests expecting status < 400 but receiving `404` and `415 Unsupported Media Type`. Ensure content-type headers like `application/json` are being sent correctly for payloads.
