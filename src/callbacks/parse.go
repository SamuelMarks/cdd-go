...
// Parse extracts an OpenAPI Callback object from a struct representation.
func Parse(decl *dst.GenDecl) (string, *openapi.Callback) {
	...
	for _, spec := range decl.Specs {
		if vs, ok := spec.(*dst.ValueSpec); ok {
			if len(vs.Names) > 0 && strings.HasPrefix(vs.Names[0].Name, "Callback") && len(vs.Values) > 0 {
				if cl, ok := vs.Values[0].(*dst.CompositeLit); ok {
					name := strings.TrimPrefix(vs.Names[0].Name, "Callback")
					if name != "" {
						name = strings.ToLower(name[:1]) + name[1:]
					}

					callback := make(openapi.Callback)
					hasContent := false
