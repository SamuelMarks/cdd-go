...
This guide outlines the best practices for publishing the output of the `cdd-go` (language-to-OpenAPI / OpenAPI-to-language) tool. Specifically, it details how to manage a generated Go client library, keep it in sync with a server's OpenAPI spec, and publish it automatically.
...
1. Your generator (`cdd-go from_openapi -i openapi.json -o .`) writes models and routes into the repo.
...
To ensure your client library accurately reflects the active server logic, you should run `cdd-go` in a CI/CD pipeline (e.g., GitHub Actions) using a cron job. The pipeline will:
1. Download the latest `openapi.json` from your live API server.
2. Run `cdd-go` to generate the new AST/classes/routes.
    ...
    steps:
      ...
      - name: Generate Go Client Code
        run: |
          ...
          cdd-go from_openapi -i openapi.json -o pkg/
